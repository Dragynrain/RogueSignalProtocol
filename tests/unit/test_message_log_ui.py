...
class TestMessageLog:
    ...
    def test_add_message_with_type(self):
        ...
        with patch.object(DataLoader, "load_config") as mock_config:
            mock_config.return_value = {
                "colors": {"message_log": {"error": [255, 0, 0], "default": [144, 238, 144]}}
            }
    ...
    def test_get_color_by_type(self):
        ...
        with patch.object(DataLoader, "load_config") as mock_config:
            mock_config.return_value = {
                "colors": {"message_log": {"warning": [255, 255, 0], "default": [144, 238, 144]}}
            }
...
class TestMessageLogIntegration:
    ...
    def test_message_color_consistency(self):
        ...
        with patch.object(DataLoader, "load_config") as mock_config:
            mock_config.return_value = {
                "message_types": {
                    "patterns": {"error": ["error", "failure"], "success": ["success", "complete"]}
                },
                "colors": {
                    "message_log": {
                        "error": [255, 0, 0],
    ...
    def test_message_log_ui_rendering_compatibility(self):
        ...
        for i, message in enumerate(log.get_recent_messages(10)):
            ...
            render_char_safe(
                mock_console, 0, i, message.text[0] if message.text else " ", fg=message.color
            )
