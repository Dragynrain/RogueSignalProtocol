...
class TestRapidInputScenarios:
    """Test rapid input scenarios and input buffering prevention."""
    
    def test_rapid_movement_input(self):
        """Test that rapid movement inputs are processed correctly."""
        game = create_mock_game()
        handler = InputHandler(game)
        
        # Simulate rapid movement inputs
        movement_events = [
            create_mock_event(tcod.event.KeySym.W),
            create_mock_event(tcod.event.KeySym.W),
            create_mock_event(tcod.event.KeySym.D),
            create_mock_event(tcod.event.KeySym.S)
        ]
        ...
        # Each movement should have been processed
        assert game.move_player.call_count == 4
    
    def test_rapid_exploit_usage(self):
        """Test rapid exploit key presses."""
        game = create_mock_game()
        game.player.inventory_manager.equipped_exploits = ["exploit1", "exploit2"]
        handler = InputHandler(game)
        ...
        # Rapid exploit key presses
        exploit_events = [
            create_mock_event(tcod.event.KeySym.N1),
            create_mock_event(tcod.event.KeySym.N2),
            create_mock_event(tcod.event.KeySym.N1)
        ]
        ...
        # Should have called exploit system 3 times
        assert handler.exploit_system.use_exploit.call_count == 3
    
    def test_rapid_ui_toggle(self):
        """Test rapid UI toggle inputs."""
        game = create_mock_game()
        handler = InputHandler(game)
        
        # Rapid inventory toggles
        inventory_events = [
            create_mock_event(tcod.event.KeySym.I),
            create_mock_event(tcod.event.KeySym.I),
            create_mock_event(tcod.event.KeySym.I)
        ]
        ...
        # Final state should be open (3 toggles: closed->open->closed->open)
        assert game.show_inventory is True


class TestInvalidInputHandling:
    """Test handling of invalid or unexpected input combinations."""
    
    def test_unknown_key_combinations(self):
        """Test that unknown key combinations don't crash the system."""
        game = create_mock_game()
        handler = InputHandler(game)
        
        # Test obscure keys that shouldn't trigger any action
        unknown_keys = [
            tcod.event.KeySym.F13,
            tcod.event.KeySym.PRINTSCREEN,
            tcod.event.KeySym.PAUSE,
            tcod.event.KeySym.MENU
        ]
        
        for key in unknown_keys:
            event = create_mock_event(key)
            result = handler.handle_keydown(event)
            assert result is True  # Should continue without crashing
            
            # Verify no unintended side effects
            game.move_player.assert_not_called()
            game.maybe_process_turn.assert_not_called()
    ...
    def test_modifier_key_combinations(self):
        ...
        # Test movement with modifiers (should still work)
        event = create_mock_event(tcod.event.KeySym.W)
        event.mod = tcod.event.Modifier.LCTRL
        ...
        event = create_mock_event(tcod.event.KeySym.A)
        event.mod = tcod.event.Modifier.LALT
    ...
    def test_null_or_malformed_events(self):
        """Test handling of malformed or null events."""
        game = create_mock_game()
        handler = InputHandler(game)
        
        # Test with missing attributes (should be handled gracefully)
        malformed_event = Mock()
        malformed_event.sym = None
        
        # Should not crash
        try:
            result = handler.handle_keydown(malformed_event)
            # If it returns, it should be True (continue)
            assert result is True
        except AttributeError:
            # If it throws AttributeError, that's also acceptable behavior
            # as it indicates the input validation is working
            pass
