...
class TestLevelGeneration:
    ...
    def test_gateway_exists_and_is_reachable(self):
        ...
        assert gateway_pos not in agent.game_map.walls, "Gateway is inside a wall"
    ...
    def test_enemies_dont_overlap_player_spawn(self):
        ...
        for enemy in agent.enemies:
            enemy_pos = (enemy.x, enemy.y)
            assert enemy_pos != player_pos, f"Enemy {enemy.type} spawned on player at {player_pos}"
    ...
    def test_map_connectivity(self, agent_with_guaranteed_gateway):
        ...
        distance_to_gateway = abs(agent.player.x - gateway_x) + abs(agent.player.y - gateway_y)
    ...
    def test_special_tiles_in_valid_positions(self):
        ...
        for node_pos in agent.game_map.cooling_nodes:
            assert (
                node_pos not in agent.game_map.walls
    ...
    def test_map_has_some_features(self):
        ...
        assert feature_count >= 5, f"Map has only {feature_count} features - may be too empty"
    ...
    def test_level_1_spawn_quantities_match_config(self):
        ...
        assert len(mismatches) == 0, "Spawn quantities don't match config:\n" + "\n".join(
            mismatches
        )
    ...
    def test_all_levels_match_config(self):
        ...
        for level_num in [1, 2, 3]:
            ...
            print(
                f"  Cooling nodes: {actual['cooling_nodes']} (expected {expected['cooling_nodes']})"
            )
            ...
            print(
                f"  Exploits: {actual['exploit_pickups']} (expected {expected['exploit_pickups']})"
            )
            print(
                f"  Upgrades: {actual['permanent_upgrades']} (expected {expected['permanent_upgrades']})"
            )
