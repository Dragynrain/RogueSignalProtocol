...
class TestPlayerEnemyVisionChain:
    ...
    def test_player_enters_enemy_vision_triggers_alert(self, basic_game_engine):
        ...
        with patch.object(basic_game_engine.player, "is_invisible", return_value=False):
            ...
            with (
                ...
                can_see = scanner1.can_see_player(
                ...
                if can_see:
                    ...
                    scanner1.last_seen_player = Position(
                        basic_game_engine.player.x, basic_game_engine.player.y
                    )

        # Verify scanner1 detected player if within range
        distance = scanner1.position.distance_to(
            Position(basic_game_engine.player.x, basic_game_engine.player.y)
        )
        if distance <= vision_range:
            assert (
                scanner1.state == EnemyState.ALERT
            ), "Scanner should become alert when seeing player"
            assert scanner1.last_seen_player == Position(
                basic_game_engine.player.x, basic_game_engine.player.y
            ), "Scanner should track player position"
    ...
    def test_enemy_alerting_chain(self, basic_game_engine):
        ...
        # Create enemies for this test
        scanner1 = enemy_builder(
            "scanner", pos=(15, 10), state=EnemyState.ALERT, last_seen=(12, 10)
        )
        scanner2 = enemy_builder("scanner", pos=(30, 10))
        ...
        if hasattr(basic_game_engine, "_alert_nearby_enemies"):
            ...
            if patrol_distance <= alert_range:
                ...
                assert patrol1.state in [
                    EnemyState.ALERT,
                    EnemyState.HOSTILE,
                ], "Nearby enemies should become alert or hostile"
    ...
    def test_alerted_enemies_update_movement_queues(self, basic_game_engine):
        ...
        moved = scanner1.move(
            basic_game_engine.game_map, basic_game_engine.player, basic_game_engine
        )

        # Verify movement behavior is valid
        if moved:
            assert not basic_game_engine.game_map.is_wall(
                scanner1.position
            ), f"Enemy moved to wall at {scanner1.position}"
            assert (
                0 <= scanner1.position.x < basic_game_engine.game_map.width
            ), "Enemy must stay within map width"
            assert (
                0 <= scanner1.position.y < basic_game_engine.game_map.height
            ), "Enemy must stay within map height"
        ...
        if scanner1.move_queue:
            ...
            assert not basic_game_engine.game_map.is_wall(
                next_planned
            ), f"Planned move to {next_planned} must not be a wall"
    ...
    def test_complete_trace_level_workflow(self, basic_game_engine):
        ...
        initial_distance = Position(
            basic_game_engine.player.x, basic_game_engine.player.y
        ).distance_to(scanner1.position)
        ...
        with (
            patch.object(basic_game_engine.player, "is_invisible", return_value=False),
            patch.object(basic_game_engine.game_map, "can_see_position", return_value=True),
            patch.object(basic_game_engine.game_map, "is_blind_spot", return_value=False),
        ):
        ...
        new_distance = Position(basic_game_engine.player.x, basic_game_engine.player.y).distance_to(
            scanner1.position
        )
        vision_range = scanner1.type_data.vision
        ...
        if new_distance <= vision_range:
            ...
            assert (
                scanner1.state == EnemyState.ALERT
            ), "Enemy should be alert after detecting player"
            ...
            assert (
                scanner1.state == EnemyState.UNAWARE
            ), "Enemy should remain unaware if player out of range"
    ...
    def test_line_of_sight_blocking(self, basic_game_engine):
        ...
        with (
            patch.object(basic_game_engine.player, "is_invisible", return_value=False),
            patch.object(basic_game_engine.game_map, "can_see_position", return_value=False),
        ):  # Blocked by wall
        ...
        assert (
            scanner1.state == EnemyState.UNAWARE
        ), "Enemy should remain unaware when vision blocked"
    ...
    def test_player_invisibility_prevents_trace_level(self, basic_game_engine):
        ...
        with (
            patch.object(basic_game_engine.player, "is_invisible", return_value=True),
            patch.object(basic_game_engine.game_map, "can_see_position", return_value=True),
            patch.object(basic_game_engine.game_map, "is_blind_spot", return_value=False),
        ):
        ...
        assert (
            scanner1.state == EnemyState.UNAWARE
        ), "Enemy should remain unaware of invisible player"
