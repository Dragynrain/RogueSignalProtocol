...
class TestResourceBalance:
    ...
    def test_exploit_heat_costs_reasonable(self):
        ...
        for exploit_id, data in exploits.items():
            ...
            assert (
                heat_cost <= 100
            ), f"Exploit '{exploit_id}' heat cost ({heat_cost}) exceeds max heat"

            # Emergency exploits should have high heat (35+ due to System Crash self-damage rebalance)
            if category == "emergency":
                assert (
                    heat_cost >= 35
                ), f"Emergency exploit '{exploit_id}' should have high heat cost"
...
class TestExploitBalance:
    ...
    def test_stealth_exploits_no_damage(self):
        ...
        for exploit_id, data in stealth_exploits.items():
            damage = data.get("damage", -1)
            assert damage == 0, f"Stealth exploit '{exploit_id}' should have 0 damage, got {damage}"
    ...
    def test_utility_exploits_no_damage(self):
        ...
        for exploit_id, data in utility_exploits.items():
            damage = data.get("damage", -1)
            assert damage == 0, f"Utility exploit '{exploit_id}' should have 0 damage, got {damage}"
    ...
    def test_area_exploits_have_reasonable_range(self):
        ...
        for exploit_id, data in area_exploits.items():
            ...
            assert (
                range_val <= 5
            ), f"Area exploit '{exploit_id}' range ({range_val}) seems too large"
