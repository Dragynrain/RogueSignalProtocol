...
**Keep both AGENTS.md files < 100 lines**
...
**Check docs BEFORE implementing:** Mouse → `MOUSE_COORDINATE_HANDLING.md`, Gamepad → `gamepad.md`, TCOD → `TCOD_GUIDE.md`, Distance → `DISTANCE_GUIDE.md`, Rendering → `RENDERING_ARCHITECTURE.md`, Testing → `TESTING_GUIDE.md`, Batch → `WINDOWS_SCRIPTING.md` (all in `.Codex/`)
...
See `.Codex/MOUSE_COORDINATE_HANDLING.md`
...
See `.Codex/RENDERING_ARCHITECTURE.md`
...
NO `else if` - use `) else ( if ... )`. Test before commit. See `.Codex/WINDOWS_SCRIPTING.md`
